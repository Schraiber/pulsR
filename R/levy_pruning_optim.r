...
fit_reml_levy = function(phy,dat,model,par=NA,sigma_tip=T,tol=1e-4,maxfun=2000,maxiter=1000,weight_scaling=1e5,silent=T,development=F, num_attempt=1) {
    ...
    for (k in 1:num_attempt) {
        if (!silent) {
            print(paste0("Attempt ", k, " of ", num_attempt))
        }
        ...
        if (model=="BM") {
            ...
            fn = function(x=NULL, index=NULL, fmsfundata=NULL) {
                ...
                if (any(x<0)) {
                    ...
                    ret = llik_reml_levy(phy,dat,"BM",sigma_bm=x[1],alpha_ou=0,decay_eb=0,sigma_tip=x[2],weight_scaling=weight_scaling,silent=silent)
                }
        ...
        # ORNSTEIN-UHLENBECK
          ...
          fn = function(x=NULL, index=NULL, fmsfundata=NULL) {
            ...
            if (any(x<0)) {
                ...
                ret = llik_reml_levy(phy,dat,"OU",sigma_bm=x[1],alpha_ou=x[2],decay_eb=0,sigma_tip=x[3],weight_scaling=weight_scaling,silent=silent)
            }
            if (any(ret == -Inf)) ret = -(0.1/tol)
            return(list(f=-sum(ret),
                        g=c(),
        ...
        # EARLY BURST
          ...
          fn = function(x=NULL, index=NULL, fmsfundata=NULL) {
            ...
            if (x[1] < 0 || x[2] > 0 || x[3] < 0) {
                ...
                ret = llik_reml_levy(phy,dat,"EB",sigma_bm=x[1],alpha_ou=0,decay_eb=x[2],sigma_tip=x[3],weight_scaling=weight_scaling,silent=silent)
            }
            if (any(ret == -Inf)) ret = -(0.1/tol)
            return(list(f=-sum(ret),
                        g=c(),
            ...
            fn = function(x=NULL, index=NULL, fmsfundata=NULL) {
                ...
                if (any(x<0)) {
                    ...
                    ret = llik_reml_levy(phy,dat,"BMJN",sigma_bm=x[1],lambda_jn=x[2],sigma_jn=x[3],
                                    alpha_ou=0,decay_eb=0,sigma_tip=x[4],weight_scaling=weight_scaling,silent=silent)
                }
                if (any(ret == -Inf)) ret = -(0.1/tol)
                return(list(f=-sum(ret),
                        g=c(),
            ...
            fn = function(x=NULL, index=NULL, fmsfundata=NULL) {
                ...
                if (any(x<0)) {
                    ...
                    ret = llik_reml_levy(phy,dat,"JN",lambda_jn=x[1],sigma_jn=x[2],
                                    alpha_ou=0,decay_eb=0,sigma_tip=x[3],weight_scaling=weight_scaling,silent=silent)
                }
                if (any(ret == -Inf)) ret = -(0.1/tol)
                return(list(f=-sum(ret),
                        g=c(),
        ...
        # EARLY BURST w/ RATE DECAYING JN
          ...
          fn = function(x=NULL, index=NULL, fmsfundata=NULL) {
            ...
            if (x[1] > 0 || x[2] <  0 || x[3] < 0 || x[4] < 0) {
                ...
                ret = llik_reml_levy(phy,dat,"EBJN",alpha_ou=0,decay_eb=x[1],lambda_jn=x[2],sigma_jn=x[3],sigma_tip=x[4],weight_scaling=weight_scaling,silent=silent)
            }
            if (any(ret == -Inf)) ret = -(0.1/tol)
            return(list(f=-sum(ret),
                        g=c(),
        ...
        # EARLY BURST w/ TIME DECAYING VG
          ...
          fn = function(x=NULL, index=NULL, fmsfundata=NULL) {
            ...
            if (x[1] > 0 || x[2] <  0 || x[3] < 0 || x[4] < 0) {
                ...
                ret = llik_reml_levy(phy,dat,"EBVG",alpha_ou=0,decay_eb=x[1],nu_vg=x[2],sigma_vg=x[3],sigma_tip=x[4],weight_scaling=weight_scaling,silent=silent)
            }
            if (any(ret == -Inf)) ret = -(0.1/tol)
            return(list(f=-sum(ret),
                        g=c(),
        ...
        # EARLY BURST w/ TIME DECAYING NIG
          ...
          fn = function(x=NULL, index=NULL, fmsfundata=NULL) {
            ...
            if (x[1] > 0 || x[2] <  0 || x[3] < 0 || x[4] < 0) {
                ...
                ret = llik_reml_levy(phy,dat,"EBNIG",alpha_ou=0,decay_eb=x[1],alpha_nig=x[2],delta_nig=x[3],sigma_tip=x[4],weight_scaling=weight_scaling,silent=silent)
            }
            if (any(ret == -Inf)) ret = -(0.1/tol)
            return(list(f=-sum(ret),
                        g=c(),
        ...
        if (k == 1) {
            k_best = 1
            nm_best = nm
        } else if (-nm$optbase$fopt > -nm_best$optbase$fopt) {
            k_best = k
            nm_best = nm
        }
    }
    ...
    if (!silent) cat ("...done!\n")
    if (!silent) cat ("MLE", k, "of", num_attempt, "kept\n")
    ...
    results$dat = dat
    results$phy = phy
    results$params = format_params(nm_best$optbase$xopt[,1], model)
    results$n_params = sum( results$params != 0 )
    results$lnL = -nm_best$optbase$fopt
    results$AIC = 2*(results$n_params-results$lnL)
    results$optim = nm_best
    return(results)
}
